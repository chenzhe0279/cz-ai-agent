...
基于 **Java 21、Spring Boot 3、Spring AI** 的 AI Agent 工程。主服务提供**通用 AI 智能助手**（后端类名与接口沿用旧命名 LoveApp、`/love_app/*`）、多步 ReAct 超级智能体（CzManus）、**AI 情感专家**（RAG 知识检索对话，`/ai/rag/chat/sse`，按情感状态过滤知识库）、丰富的本地工具调用、RAG 检索增强、Human-in-the-loop（人机交互）与完整的用户体系（注册/登录/个人中心/邮箱验证/找回密码/VIP/管理员），并通过 MCP（stdio）集成图片检索子服务 `cz-image-search-mcp-server`。仓库还包含一个 Vue 3 前端 `cz-ai-agent-frontend`，以 SSE 流式方式与后端对话；界面以**四套可切换的动态背景**（蜡笔小新·湖边之夜 / 彗星日落视频 / 星空流星 / 彗星蓝天·你的名字，**默认初始背景为蜡笔小新·湖边之夜**，右下角按钮切换并支持自动轮播）为基调，主页采用完全透明、登录/注册/找回密码与聊天页、个人中心采用毛玻璃通透风格。
...
│   ├── public/background/      # 全局动态背景资源：xiaoxin-night.webp（蜡笔小新·湖边之夜，默认初始背景）+ comet.mp4（彗星日落视频）+ stars.webp / starry-eyes.webp（星空流星 / 彗星蓝天·你的名字图片）+ 首帧占位图
...
- **入场显现动画**：首页打开后，文案按「主导航 → 眉题 → 大标题 → 副文案 → CTA → 应用卡片 → 特性条 → 页脚」的 DOM 顺序**自上而下缓缓浮现**（每个区块约 0.9~1s，交错节奏全流程约 1.8s）。纯 CSS 实现：`.reveal` 通过 `reveal-up` 关键帧控制 `opacity / translateY / 轻微 blur`，让文字呈现「聚焦浮现」质感；使用 `animation-fill-mode: backwards`，仅在延迟期间保持隐藏帧、动画结束后回落到自然样式，因此**不遮挡卡片 / 按钮的 hover 位移**；各区块用 CSS 变量 `--rd` 设置各自的 `animation-delay`；并遵循 `prefers-reduced-motion`，弱化动态偏好的用户直接展示、不播放入场动画。深空星点 / 极光 / 网格等装饰层保持隐藏以透出背景，不在登场序列内。
...
- **背景切换**：右下角悬浮控件支持在「蜡笔小新·湖边之夜（默认初始背景）/ 彗星日落（视频）/ 星空流星 / 彗星蓝天·你的名字」四套背景间切换（‹ › 或点击名称），并可通过 ▶/❚❚ 按钮开启/关闭**自动轮播**（10 秒切换一次）；选择与轮播开关持久化到 `localStorage`，刷新后保持。蜡笔小新·湖边之夜、星空流星与彗星蓝天为静态壁纸，前端以缓慢缩放平移（Ken Burns）动效呈现流动感。
...
- **视觉风格**：全站以**四套可切换动态背景**为主角（蜡笔小新·湖边之夜、星空流星与彗星蓝天·你的名字为 Ken Burns 动效图片，彗星日落为视频；**默认初始背景为蜡笔小新·湖边之夜**，全局固定一层、所有页面共用，右下角按钮切换）；主页为**轻玻璃**（8~10px 轻微模糊 + 极淡底色，兼顾通透与可读性），登录/注册/找回密码、聊天页、个人中心为**毛玻璃通透**（轻微 6~12px 模糊 + 极淡底色）；深空星点、极光、行星等装饰层已弱化以透出背景。
